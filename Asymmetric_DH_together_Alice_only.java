...
public class Asymmetric_DH_together_Alice_only{
...
class Experiment{
    ...
    while (scanner.hasNextInt()) {  // 標準入力に次の整数がある限り実行
        ...
        // keysize と d を文字列に反映
				String OUTPUT = String.format("ADH_k=24SEDH_%d_bit_d_%d_m_%d.csv", keysize, d, bitSize_m);
				    ...
				    for (int j = 0; j < NUM_RUN; j++) {
						...
						for (int j = 0; j < NUM_RUN; j++) {
								...
								try {
										...
										printResurt(pw, d, p, bitSize_m, run(d, p, g, bitSize_m, bList[j], yB1List[j], yB2List[j], baseList[j]),averageCpuLoad,coresUsed);
	...
	private static long[] run(int d, BigInteger p, BigInteger g, int bitSize_m, BigInteger b, BigInteger[] yB1, BigInteger[] yB2, BigInteger[] NB)
